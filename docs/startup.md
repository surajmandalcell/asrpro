# ASR Pro Startup Launch

> TLDR: use Configuration, Application, `Launch at startup`. Turning it on writes the current executable path, so it replaces the saved startup target when the app has moved.

## Intent

Startup launch should be simple for normal users and repairable for portable-style installs. If someone moves ASR Pro from one folder to another, enabling startup again should point the operating system at the new executable.

| Behavior | Detail |
|---|---|
| Toggle on | Registers the current executable for login startup. |
| Toggle off | Removes the saved ASR Pro startup entry for this install. |
| Toggle on again after moving | Replaces the saved startup target with the current executable path. |
...
| Startup points at an old copy | Toggle off, then toggle on. The next enabled state replaces the saved startup target. |
...
| Rewrites the startup target when enabled from a new path. | Move `asrpro-data/` for the user. |
