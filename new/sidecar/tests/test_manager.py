...
class TestModelManager:
    ...
    async def test_initialize(self, settings):
        ...
        with patch.object(manager.device_detector, 'detect_capabilities', new_callable=AsyncMock) as mock_detect:
    ...
    async def test_list_available_models(self, settings):
        ...
        with patch.object(manager.registry, 'list_models', return_value=['whisper-base', 'parakeet-ctc']):
    ...
    def test_get_current_device(self, settings):
        ...
        with patch.object(manager.device_detector, 'get_current_device', return_value='cuda'):
    ...
    async def test_set_model_success(self, settings):
        ...
        with patch.object(manager.registry, 'is_model_available', return_value=True):
            with patch.object(manager.registry, 'get_loader_type', return_value='whisper'):
                with patch.object(manager.registry, 'get_model_info', return_value={'id': 'whisper-base'}):
                    with patch.object(manager, '_get_loader', return_value=Mock()) as mock_get_loader:
                        mock_loader = Mock()
    ...
    async def test_get_loader_new_whisper(self, settings):
        ...
        manager = ModelManager(settings)
        manager.loader_configs = {'whisper': {'device': 'cpu', 'compute_type': 'float32'}}
    ...
    async def test_get_loader_new_parakeet(self, settings):
        ...
        with patch.object(manager.registry, 'get_loader_type', return_value='parakeet'):
            with patch.object(manager.registry, 'get_model_info', return_value={'id': 'parakeet-ctc'}):
    ...
    async def test_transcribe_file_success(self, settings, sample_audio_file):
        ...
        manager.current_model = 'whisper-base'
        manager.current_loader = Mock()
        manager.current_loader.transcribe = AsyncMock(return_value={'text': 'Hello world'})
    ...
    async def test_unload_all_models(self, settings):
        ...
        manager.loaders = {
            'whisper-base': mock_loader1,
            'parakeet-ctc': mock_loader2
        }
        manager.current_model = 'whisper-base'
    ...
    async def test_get_model_info_not_loaded(self, settings):
        ...
        with patch.object(manager.registry, 'get_model_info', return_value={'id': 'whisper-base', 'loaded': False}):
            info = await manager.get_model_info('whisper-base')
    ...
    async def test_cleanup(self, settings):
        ...
        with patch.object(manager, 'unload_all_models', return_value=True) as mock_unload:
